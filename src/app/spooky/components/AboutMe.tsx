...
import { letterUrl } from '@components/landing/letters';
...
/* The letters themselves were renamed after this page was retired, so the
   hardcoded `aaron-everly.pdf` paths it shipped with now 404. `letterUrl` is
   where the current site spells that convention out, and it is the one thing
   here that has to keep up with the files rather than stay frozen. */
const Letters = () => (
  <div className={'w-full flex flex-col mt-6 md:px-8 font-normal text-[1.25rem] justify-center'}>
    ...
    <div className={styles.downloadContainer}>
      ...
      />
      <DownloadRow
        url={letterUrl('Andrei Budoi')}
        name={'Andrei Budoi'}
        title={'Legit Script: Team Lead'}
        tabIndex={7}
      />
      <DownloadRow url={letterUrl('Hines Tran')} name={'Hines Tran'} title={'Shift Pixy: Product Owner'} tabIndex={6} />
      <DownloadRow
        url={letterUrl('Steve Swanson')}
        name={'Steve Swanson'}
